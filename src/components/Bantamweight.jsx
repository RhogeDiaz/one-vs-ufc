...
const Bantamweight = () => {
    const [bantamONE, setBantamONE] = useState([]);
    const [bantamUFC, setBantamUFC] = useState([]);
    ...
    const voteONE = async (fighterName) => {
        ...
        if (updateError) {
            ...
            setBantamONE(prevBantamONE =>
                prevBantamONE.map(fighter =>
    ...
    const voteUFC = async (fighterName) => {
        ...
        if (updateError) {
            ...
            // Update the state to reflect the new bets value
            setBantamUFC(prevBantamUFC =>
                prevBantamUFC.map(fighter =>
                    fighter.name === fighterName ? { ...fighter, bets: newBets } : fighter
                )
            );
        }
