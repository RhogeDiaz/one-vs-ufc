...
const Featherweight = () => {
    const [featherONE, setfeatherONE] = useState([]);
    const [featherUFC, setfeatherUFC] = useState([]);
    ...
    const voteONE = async (fighterName) => {
        ...
        if (updateError) {
            ...
            setfeatherONE(prevfeatherONE =>
                prevfeatherONE.map(fighter =>
    ...
    const voteUFC = async (fighterName) => {
        ...
        if (updateError) {
            ...
            // Update the state to reflect the new bets value
            setfeatherUFC(prevfeatherUFC =>
                prevfeatherUFC.map(fighter =>
                    fighter.name === fighterName ? { ...fighter, bets: newBets } : fighter
                )
            );
        }
    ...
    return (
        <main className="flex flex-col items-center" >
            ...
            <h1 className="font-bold text-2xl p-4">FEATHERWEIGHT</h1>
